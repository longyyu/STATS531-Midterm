...
[TODO: (1) Check the interpretation of the CCF plot and the analysis following the centered water level plot; (2) I am not sure what the symbol `**` in the introduction means, or the $d\in[5]$ in the Trend section. Feel free to keep them as long as they are not typos. ]{.comment}
...
We start by choosing an appropriate SARMA model, again using AIC as a comparison metric. To keep the model relatively simple, we limit the seasonal AR and MA factors $(P,Q)$ to $\{(0,0), (1,0), (0,1), (1,1)\}$, and find that $(P, Q) = (1, 1)$ has much lower AIC values than the other three pairs. 
...
Again, we compute the roots of the AR polynomial $\phi(x)\Phi(x^{12}) = (1-1.369x+0.394x^2)(1-x^{12})$ and MA polynomial $\Psi(x^{12}) = 1-0.974x^{12}$. All roots are outside the unit circle, indicating that the model is both stationary and invertible. 
...
[The paragraph below may not be accurate: The plot presents the centered data; if we compare the original data, Lake Michigan-Huron's water level has always been higher than Lake Erie's. ]{.comment}
...
[The maximum value should correspond to lag 1?]{.comment}

```{r, echo=TRUE}
# TODO: delete this later!
ccf_val = ccf(data_huron$MSL, data_erie$MSL, plot = FALSE)
ccf_val$lag[which.max(ccf_val$acf)]
```
