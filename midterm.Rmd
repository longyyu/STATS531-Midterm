...
From the time plot below, we can see that the water levels had two noticeable peaks in October 1986 and July 1997, hit bottom in 2013, and have been steadily rising since then. 
...
date_num = seq(from = 1980, length = nrow(data_huron), by = 1/12)
data_huron %>%
  mutate(Residuals = lm(level ~ poly(date_num, 5))$residuals) %>%
...
```{r echo=FALSE, fig.width = 6, fig.height = 3}
# Model 2: ARIMA
# m2 = arima(level, c(3, 1, 3), method='ML')
