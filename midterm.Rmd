...
- [Limitations] [refine this section]{.comment}
...
In this project, we consider the variation of water levels in Lake Michigan-Huron over the time period 1980-2020, given that the majority of residents in the Great Lakes watershed live by Lake Michigan [[4](https://www.watershedcouncil.org/great-lakes-water-levels.html), [5](https://en.wikipedia.org/wiki/Lake_Michigan%E2%80%93Huron)]. Our goal is to identify if there is a general trend that could explain the noticeable increase in water levels in the past 10 years, and to describe any other patterns that may be of interest to environmental scientists and climatologists. In addition, we may want to study how the water levels in one lake may be associated with the water levels in another lake for prediction purposes; thus, we will cross-compare water levels between Lake Michigan and Lake Erie.
...
$$
\phi(B)\Phi_1(B^{12})\Phi_1(B^{100})\left[Y_n-(\beta_0 + \beta_1t_n + \beta_2t_n^2 + \beta_3t_n^3)\right] = \psi(B)\Psi_1(B^{12})\Psi_1(B^{100})\varepsilon_n
$$

where $\{\varepsilon_n\}$ is a Gaussian white noise process, $B$ is the backshift operator, and $\phi(\cdot)$, $\Phi(\cdot)$, $\psi(\cdot)$ and $\Psi(\cdot)$ are the monthly and yearly AR and MA polynomials, respectively.
...
We start by choosing an appropriate SARMA model using Akaike's information criterion (AIC) as a comparison metric.
...
data_huron %>%
  ...
  ggplot() + 
    ...
    geom_line(aes(Date, fit), color='#F8766D', lty=2, size=1) +
    ylab("Mean Lake Level (ft)") +
    ggtitle('Lake Michigan-Huron Water Levels, 1980-2020') +
    scale_x_date(breaks=date_breaks, date_labels='%Y') +
    theme(plot.title=element_text(hjust=0.5))
...
Let us also compare the two series by investigating the cross-correlation; if there is a noticeable pattern, it may be worth analyzing further.

```{r echo=FALSE}
ccf(data_huron$MSL, data_erie$MSL, main="sample cross-correlation function")
```
...
From comparing Lake Erie's water levels to Lake Michigan's, we see that the two lakes tend to vary together in time. They have the same seasonal pattern and general trend, which makes sense due to similar rainfall patterns in the Great Lakes region. Surprsingly, Lake Erie's water levels have fluctuated a bit less than Lake Michigan's, despite its smaller size and depth [[7](https://en.wikipedia.org/wiki/Lake_Erie)]. While the data in one lake do not lag in another, it is valuable to note that what affects one lake, also seems to affect the other.
...
We ran into numerical stability problems on occasion where our model fitting algorithm did not converge. This gives us evidence that some of our model choices were not entirely well-specified. A more careful analysis may be of use, especially one that verifies our assumptions with bootstrapping.
