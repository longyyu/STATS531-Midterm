...
stl(ts(level, start = c(1980, 2), frequency=12), s.window = 12) %>% plot
```

```{r, echo=TRUE, fig.width = 6, fig.height = 5}
# transform Date to numeric type
date_num = seq(from = 1980 + 1/12, length = nrow(data_mich), by = 1/12)
...
```{r echo=FALSE, fig.width = 6, fig.height = 3}
# Model 3: SARMA
m3 = arima(level, c(5, 0, 3), method='ML',
           seasonal = list(order = c(1, 0, 0), period = 12))
...
```{r echo=FALSE}
generate_aic_table(level, P=5, Q=5, D=0, xreg=data_erie$MSL) %>%
...
m4 = arima(level, c(5, 0, 2), method='ML', xreg=data_erie$MSL)
